...
const PLUGINS = [
  ...
  },
  {
    ...
    title: "Mirror",
    category: "SIGNAL",
    desc: "Reads the person inside the field. Interprets what your results mean personally.",
          ...
          <div className="sticky top-24 space-y-10">
            <div>
              ...
              <div className="space-y-1 border-l border-[var(--color-gold)]/20 pl-4">
                {["ALL", "MIRROR", "SIGNAL", "MAP", "MOVE"].map((cat) => (
                  <div
                    key={cat}
            ...
            <div>
              ...
              <div className="space-y-1 border-l border-[var(--color-gold)]/20 pl-4">
                {["INSTALLED", "AVAILABLE", "UPDATES"].map((stat) => (
                  <div
                    key={stat}
                    className="font-display text-lg text-[var(--color-parchment)]/50 hover:text-[var(--color-parchment)] cursor-pointer py-1 transition-all"
                  >
                    {stat}
                  </div>
                ))}
        ...
        <div className="flex-1">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredPlugins.map((plugin) => {
              ...
              const effectiveLink = mirrorDormant ? "#" : plugin.link;
              const effectiveImage = isMirror
                ? mirrorRevealed
                  ? plugin.imageRevealed
                  : plugin.imageDormant
                : plugin.image;

              return (
                <Link key={plugin.id} href={effectiveLink}>
                  <div
                    className={cn(
                      "group relative bg-[var(--color-card)] border transition-all duration-500 flex flex-col h-full",
                      mirrorRevealed
                        ? "border-[rgba(197,160,89,0.5)] cursor-pointer hover:-translate-y-1 hover:shadow-[0_0_40px_-8px_rgba(197,160,89,0.4)] hover:border-[var(--color-gold)]"
                        : mirrorDormant
                        ? "border-[var(--color-gold)]/10 opacity-80 cursor-default"
                        : plugin.status === "LOCKED"
                        ? "border-[var(--color-gold)]/20 opacity-70 grayscale-[0.5] cursor-pointer"
                        : "border-[var(--color-gold)]/20 cursor-pointer hover:-translate-y-1 hover:shadow-[0_0_30px_-10px_rgba(197,160,89,0.2)] hover:border-[var(--color-gold)]/50"
                    )}
                  >
                    {/* Corner flourishes */}
                        ...
                        <img
                          ...
                          className={cn(
                            ...
                            mirrorDormant
                              ...
                              : "opacity-80 group-hover:opacity-100 group-hover:scale-105 sepia-[0.4] group-hover:sepia-0"
                          )}
                        />
                      ) : (
                        ...
                        <span
                          className={cn(
                            ...
                            plugin.status === "ACTIVE"
                              ...
                              : mirrorRevealed
                              ? "bg-[rgba(197,160,89,0.15)] border-[rgba(197,160,89,0.6)] text-[var(--color-gold)]"
                              : mirrorDormant
                              ? "bg-[var(--color-card)]/80 border-[var(--color-gold-dim)]/30 text-[var(--color-gold-dim)]/40"
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      <div className="pt-4 mt-4 border-t border-[var(--color-gold)]/20 flex justify-between items-center opacity-60 group-hover:opacity-100 transition-opacity duration-300">
                        <span className="text-[10px] font-pixel text-[var(--color-gold-dim)]">
                          REBEL OS
                        </span>
